...
class NewsController extends Controller
{
    ...
    public function getAllWhereOA()
    {
        ...
        $query = $this->getDatabase()->prepare("select n.id, title, link, category, description, file_name, n.creation_date, concat(a.firstname, ' ', a.lastname) as publisher from news n inner join admins a where n.publisher_id = a.id and upper(title) like upper( :search ) order by title asc;");
        $query->setFetchMode(PDO::FETCH_CLASS, News::class);
        $query->execute(array(':search' => '%' . $action . '%'));
        $newsList = $query->fetchAll();
