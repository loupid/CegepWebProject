...
<div class="mt-8 flex justify-end">
    <a href="<?=$this->router->generate('eventCreate') ?>" class="mx-2">
            ...
            <table class="min-w-full">
                ...
                <?php foreach ($eventsList as $event){?>
                    <tr>
                        ...
                        <td class="px-6 py-4 whitespace-no-wrap border-b border-gray-200">
                            <div class="text-sm leading-5 text-gray-900"><?= $event->category; ?></div>
                        </td>
                        ...
                        <td class="px-6 py-4 whitespace-no-wrap border-b border-gray-200">
                            <span class="px-2 inline-flex text-xs leading-5 font-semibold rounded-full <?= (!$event->hide)? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800'?>"><?= (!$event->hide)? "Publiée" : "Non publiée"; ?></span>
                        </td>
                        ...
                        <td class="flex justify-center px-6 py-4 whitespace-no-wrap text-right border-b border-gray-200 text-sm leading-5 font-medium">
                            <a href="<?= $this->router->generate('eventEdit', ['id'=>$event->id]) ?>" class="mx-2 text-gray-200 dark:text-gray-200 hover:text-indigo-200 dark:hover:text-gray-400">
                                <svg xmlns="http://www.w3.org/2000/svg" class="h-6 w-6 fill-current" viewBox="0 0 24 24">
                            ...
                            <a href="<?= $this->router->generate('eventDelete', ['id'=>$event->id]) ?>" class="mx-2 text-gray-200 dark:text-gray-200 hover:text-red-300 dark:hover:text-gray-400">
                                <svg xmlns="http://www.w3.org/2000/svg" class="h-6 w-6 fill-current" viewBox="0 0 24 24">
