<form action="<?php echo $this->router->generate("newsRecherche"); if ($ordreActif === "nom") {
    echo "-oa";
}?>" method="post">
...
if (!empty($newsList)) {
    ...
    <section class="mt-5 w-1/2 mx-auto">
        <div class="mx-auto pt-5 border-b border-gray-200 flex justify-center">
            ...
            <a href=".<?= $this->router->generate("newsRecherche")?>" class="mx-5 text-gray-600 transition-colors pb-3 border-b cursor-pointer <?php if ($ordreActif === "") {
        echo "border-red-500 text-red-500";
    } else {
        echo "hover:text-black";
    } ?>">Date de l'actualité</a>
            <a href=".<?= $this->router->generate("newsRechercheOA", ["search"=> $search ?? ""])?>" class="text-gray-600 transition-colors pb-3 border-b cursor-pointer <?php if ($ordreActif === "nom") {
        echo "border-red-500 text-red-500";
    } else {
        echo "hover:text-black";
